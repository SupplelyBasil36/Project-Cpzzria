...
bool validaAgrega(FILE *arch, int valor, repartidor *pR){
	...
	while(!feof(arch)){
		...
		if(valor == pR->id_repartidor){
			printf("El Se encontrÔøΩ coin %d", valor);
			bandera = true;
		}
	}
	fclose(arch);
	...
	if (primero != NULL)
	{
		while (aux2 != ultimo && encontrar != true)
		{
			if (aux1->id_repartidor == valor)
			{
				...
				printf("\a\tEl Id que ingresaste ya estÔøΩ registrado, intenta de nuevo\n\t");
				system("pause");
...
bool validaPedido(int valor)
{
	...
	if (primeroP != NULL)
	{
		while (aux2 != ultimoP && encontrar != true)
		{
			...
			{
				SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),14);
				printf("\a\tEl id de pedido que ingresaste ya est· registrado, intenta de nuevo\n\t");
				system("pause");
				SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),12);
				encontrar = true;
			}
